"""Tests for the search_usda @function_tool wrapper."""
...
async def test_search_usda_returns_candidates() -> None:
    ...
    tool = make_search_usda(fake)

    results = await tool("chicken breast")
...
async def test_search_usda_empty_returns_empty_list() -> None:
    ...
    results = await tool("xyzzy_food_does_not_exist")
...
async def test_search_usda_passes_data_type_filter() -> None:
    ...
    results = await tool("banana", data_type="Foundation")

    # FakeUSDAClient should have been called with the right data_type
